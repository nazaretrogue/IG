...
bool Escena::teclaPulsada( unsigned char tecla, int x, int y )
{
	...
	switch(tecla)
	{
		...
		case 'C':
		{
			camara_activa = (camara_activa+1)%3;
			redimensionar(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT));
		}
			break;
		case 'M':
		{
			angulo_giro_z = (angulo_giro_z + 15)%360;
			camaras[camara_activa].rotarEnZ(angulo_giro_z);
		}
			break;
	}
	return false;
...
void Escena::teclaEspecial( int Tecla1, int x, int y )
{
	switch ( Tecla1 )
	{
		...
		case GLUT_KEY_PAGE_UP:
			camaras[camara_activa].derechaEnY();
			break;
		case GLUT_KEY_PAGE_DOWN:
			camaras[camara_activa].izquierdaEnY();
			break;
	}
...
void Escena::change_projection( const float ratio_xy )
{
	...
	const float wy = 0.84 * Front_plane,
		    	wx = ratio_xy * wy;
	if(!camaras[camara_activa].esOrtogonal() && !camaras[camara_activa].examen())
		camaras[camara_activa].proyeccionPerspectiva(-wx, +wx, -wy, +wy, Front_plane, Back_plane);

	else if(!camaras[camara_activa].esOrtogonal() && camaras[camara_activa].examen())
	{
		camaras[camara_activa].proyeccionPerspectiva(-wx, +wx, -wy, +wy, Front_plane, Back_plane);
		camaras[camara_activa].cambiarVisualizacion();
	}
...
void Escena::seleccionarAspa(unsigned char pixel[3])
{
	switch (pixel[0])
	{
		case 100:
		{
			...
			std::cout << "Seleccionada aspa 1" << std::endl;
		}
			break;
		...
		case 130:
		{
			...
			std::cout << "Seleccionada aspa 2" << std::endl;
		}
			break;
		...
		case 160:
		{
			...
			std::cout << "Seleccionada aspa 3" << std::endl;
		}
			break;
		...
		case 190:
		{
			...
			std::cout << "Seleccionada aspa 4" << std::endl;
		}
			break;
	}
}
