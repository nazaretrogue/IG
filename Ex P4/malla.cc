...
void ObjPLY::calcularCentrosCaja()
{
	for(int i=0; i<vertices.size(); i++)
	{
		...
		if(z_min > vertices[i](2)) z_min = vertices[i](2);
	}

	x_total = x_max - x_min;
	y_total = y_max - y_min;

	cara_frontal = {x_total/2, y_total/2, z_max};
	cara_trasera = {x_total/2, y_total/2, z_min};
}

void ObjPLY::calcularNormales()
{
	Tupla3f vect1, vect2, tmp;
	Tupla3f tupla_0 = {0, 0, 0};
	...
	for(int i=0; i<triangulos.size(); i++)
	{
		...
		// Si las z son negativas
		if(vertices[i](2) < 0)
			n_vertices[i] = vertices[i] - cara_frontal;

		// Si las z son positivas
		else
			n_vertices[i] = vertices[i] - cara_trasera;
	}
}
