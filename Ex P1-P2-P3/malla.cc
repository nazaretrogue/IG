...
{
	vertices = {{0,1.5,0},		// 0 CÚSPIDE	
				{1,0,0},		// 1 ESQUINA DERECHA
				{0.25,0,0.25},	// 2 RINCÓN SUP DERECHA
				{0,0,1},		// 3 ESQUINA SUP
				{-0.25,0,0.25},	// 4 RINCÓN SUP IZQUIERDA
				{-1,0,0},		// 5 ESQUINA IZQUIERDA
				{-0.25,0,-0.25},// 6 RINCÓN INF IZQUIERDA
				{0,0,-1},		// 7 ESQUINA INF
				{0.25,0,-0.25},	// 8 RINCÓN INF DERECHA
				{0,0,0}};		// 9 CENTRO

	triangulos = {{0,7,8},{0,8,1},{0,1,2},{0,2,3},{0,3,4},{0,4,5},{0,5,6},{0,6,7}, // triangulos pirámide

				  {9,7,8},{9,8,1},{9,1,2},{9,2,3},{9,3,4},{9,4,5},{9,5,6},{9,6,7}}; // triángulos base
}

// *****************************************************************************
//
// Clase ObjRevolucion2
//
// *****************************************************************************

// Constructor por defecto

ObjRevolucion2::ObjRevolucion2() {}

// *****************************************************************************
// objeto de revolución obtenido a partir de un perfil (en un PLY)

ObjRevolucion2::ObjRevolucion2(const std::string & nombre_ply_perfil, bool tapa_sup, bool tapa_inf)
{
	std::vector<Tupla3f> perfil_original;

	ply::read_vertices(nombre_ply_perfil, perfil_original);
	crearMalla2(perfil_original, 20, tapa_sup, tapa_inf);
}

// *****************************************************************************
// objeto de revolución dentado
void ObjRevolucion2::crearMalla2(const std::vector<Tupla3f> &perfil_original,
							   	 const int num_instancias_perf,
							     bool tapa_sup, bool tapa_inf)
{
	int M = perfil_original.size();
	...
	for(int i=0; i<num_instancias_perf; i++)
	{
		for(int j=0; j<M; j++)
		{
			if(i%2==0)
			{
				vert(0) = 0.9*(perfil_original[j](0)*cos((2*i*M_PI)/num_instancias_perf));
				vert(1) = perfil_original[j](1);
				vert(2) = 0.9*(perfil_original[j](0)*sin((2*i*M_PI)/num_instancias_perf));
			}

			else
			{
				vert(0) = 1.1*(perfil_original[j](0)*cos((2*i*M_PI)/num_instancias_perf));
				vert(1) = perfil_original[j](1);
				vert(2) = 1.1*(perfil_original[j](0)*sin((2*i*M_PI)/num_instancias_perf));
			}
