...
Escena::Escena()
{
	...
	piramideEx = new PiramideEx();
	peon_dentado = new ObjRevolucion2("./plys/peon.ply", true, true);
	arania = new ObjJerarquico();
...
void Escena::dibujar_objeto_actual()
{
   ...
   // valor entero en 'objeto_actual'
	...
	switch( objeto_actual )
	{
		...
		case 7:
			if(piramideEx != nullptr) piramideEx->draw(modo, inmediato);
			break;

		case 8:
			if(peon_dentado != nullptr) peon_dentado->draw(modo, inmediato);
			break;

		case 9:
			if(arania != nullptr) arania->draw(modo, inmediato);
			break;
